# audits/checks/criteria/p1/c_1_4_3_contrast_minimum.py
...
def _hex_to_rgb(value: str) -> Optional[Tuple[int, int, int]]:
    try:
        ...
        if len(v) == 7:  # #RRGGBB
            ...
            b = int(v[5:7], 16)
            return (r, g, b)
        # Soporte opcional #RGBA / #RRGGBBAA → ignoramos alpha, devolvemos RGB
        if len(v) == 5:  # #RGBA
            r = int(v[1] * 2, 16)
            g = int(v[2] * 2, 16)
            b = int(v[3] * 2, 16)
...
def _parse_css_color(s: Optional[str]) -> Optional[Tuple[int, int, int, float]]:
    ...
    if m2:
        ...
        a_str = m2.group(4)  # puede ser None
...
def _font_is_bold(item: Dict[str, Any]) -> bool:
    # acepta flags o font-weight numérico >= 600
...
def _is_large_text(item: Dict[str, Any]) -> bool:
    ...
    if isinstance(fs, (int, float)):
        ...
        if m:
            ...
            # si viene en rem/em y tenemos root=16px aproximado
...
def _is_exempt_text(item: Dict[str, Any]) -> bool:
    """
    Excepciones de 1.4.3:
      - inactivo/disabled (p.ej., disabled/aria-disabled)
...
def _extract_colors(item: Dict[str, Any]) -> Tuple[Optional[Tuple[int,int,int]], Optional[Tuple[int,int,int]], Dict[str, Any]]:
    ...
    Busca en: computed_color, color, style_color; computed_background_color, background-color, bg_color, etc.
    """
    meta: Dict[str, Any] = {}
    # color de texto
    col_rgba = (
    ...
    # fallback: muchos extractores ponen 'effective_bg_color' ya resuelto
    eff_bg = _parse_css_color(item.get("effective_bg_color"))
    if eff_bg:
        bg_rgba = eff_bg

    # defaults: si no hay bg, no forzamos blanco (para no producir falsos negativos) → devolvemos None
...
def _collect_text_candidates(ctx: PageContext) -> List[Dict[str, Any]]:
    """
    Recoge candidatos con texto visible desde distintas colecciones del extractor:
      - text_nodes, headings, paragraphs, buttons, links, labels, form_controls
      - cada item debería traer: text, computed_color, computed_background_color, font_size, font_weight, etc. (si es posible)
    Si el contexto solo trae Tags BS4 sin estilos, igualmente devolvemos items con text pero colores desconocidos.
    """
    ...
    for src in ("text_nodes","headings","paragraphs","buttons","links","labels","form_controls"):
        for n in _as_list(getattr(ctx, src, [])):
            ...
            # nombre de la fuente para depurar
            item["__source"] = src
            # texto
            if not item.get("text"):
                # intenta los alternativos comunes
                for k in ("inner_text","label_text","value","aria-label","title","placeholder"):
                    if n and not isinstance(n, dict):
                        # si es Tag, intenta .get(k)
                        try:
                            v = n.get(k)  # type: ignore[attr-defined]
                            if v:
                                item["text"] = v
                                break
                        except Exception:
...
def _compute_counts_raw(ctx: PageContext) -> Dict[str, Any]:
    """
    RAW: mide contraste para texto e imágenes de texto (si se marca 'is_image_of_text').
    Reglas WCAG 1.4.3:
      - Texto normal: ≥ 4.5:1
      - Texto de tamaño grande (o bold ≥14pt ≈ 18.66px): ≥ 3:1
      Excepciones: inactivo/disabled, decorativo/incidental, logotipos.
    """
    ...
    # Imágenes de texto (si el extractor las marca)
    image_text_offenders = 0
    for im in imgs:
        # buscamos flags comunes
        flag = None
        if isinstance(im, dict):
            flag = im.get("is_image_of_text") or im.get("image_of_text")
        else:
            try:
                flag = getattr(im, "get", lambda *a, **k: None)("data-image-of-text")
            except Exception:
                flag = None
        if not flag:
            continue
        # Si el extractor no aporta colores, lo dejamos para revisión manual (no podemos medir)
        image_text_offenders += 1
        offenders.append({
            "type": "image_of_text",
            "src": _get_attr_str(im, "src")[:180],
            "reason": "Imagen de texto detectada (1.4.3 aplica). Requiere contraste equivalente o alternativa.",
        })
    ...
    details: Dict[str, Any] = {
        ...
        "image_of_text_flags": image_text_offenders,
        "ok_ratio": ok_ratio,
        "offenders": offenders,
        "note": (
            "RAW: 1.4.3 mide contraste texto/fondo. Requisitos: 4.5:1 (texto normal) y 3:1 (texto grande/bold ≥14pt≈18.66px). "
            "Se excluyen componentes inactivos, elementos decorativos/incidental y logotipos. "
            "Si faltan colores, el ítem queda como 'unknown_colors' para revisión manual."
        )
    }
    return details
...
def _compute_counts_rendered(rctx: Optional[PageContext]) -> Dict[str, Any]:
    ...
    Con DOM post-render (Playwright) puedes aportar:
      - computed_color / computed_background_color reales
      - font-size/weight reales
      - effective_bg_color (resuelto tras herencia y capas)
      - bounding boxes para excluir texto fuera de viewport/oculto
    Este método reutiliza la lógica RAW con rctx.
    """
    if rctx is None:
        return {"na": True, "note": "No se proveyó rendered_ctx para 1.4.3; no se pudo evaluar en modo renderizado."}
    d = _compute_counts_raw(rctx)
    d["rendered"] = True
    d["note"] = (d.get("note","") + " | RENDERED: estilos computados y fondo efectivo usados para medir contraste real.").strip()
    return d
...
def _ai_review(details: Dict[str, Any], html_sample: Optional[str] = None) -> Dict[str, Any]:
    """
    IA: propone ajustes de color para alcanzar 4.5:1 (o 3:1 si large).
    Sugerencias típicas:
      - Oscurecer color del texto o aclarar el fondo (o viceversa).
      - Usar variables CSS/tema con tokens de contraste AA.
      - Evitar textos sobre imágenes/gradientes sin overlay.
    """
    if ask_json is None:
        return {"ai_used": False, "ai_message": "IA no configurada.", "manual_required": False}

    offs = [o for o in details.get("offenders", []) if o.get("reason") == "Contraste insuficiente."]
    if not offs:
    ...
    prompt = (
        "Actúa como auditor WCAG 1.4.3 (Contrast mínimo). "
        "Para cada offender con 'fg' y 'bg', sugiere ajustes (CSS) para cumplir AA: "
        "a) modifica color de texto o de fondo; b) propone tokens/variables; c) sugiere overlays si hay imagen. "
...
def run_1_4_3(
    ...
    return CriterionOutcome(
        ...
        title=meta.get("title", "Contraste (mínimo)"),
        source=src,
