# audits/checks/criteria/p1/c_1_4_5_images_of_text.py
...
def _looks_like_image_of_text(el: Any) -> bool:
    ...
    if _CLASS_TEXT_HINTS.search(cls):
        return True
    # Alt muy corto (1–2 palabras) con filename sospechoso → riesgo
...
def _is_logo_like(el: Any) -> bool:
    """
    Excepción permitida por 1.4.5: logotipos.
...
def _is_customizable_exception(el: Any) -> bool:
    """
    Excepción permitida por 1.4.5: el usuario puede personalizar la presentación del texto (p. ej. estilos del usuario).
    Difícil de automatizar → acepta flags del extractor si los hay.
    """
    if isinstance(el, dict) and (_bool(el.get("user_customizable")) or _bool(el.get("is_customizable"))):
        return True
    return False

def _is_essential_exception(el: Any) -> bool:
    """
    Excepción permitida por 1.4.5: presentación específica es esencial (p. ej., logotipos, tipografía caligráfica en una obra).
    Solo por flag del extractor.
    """
    if isinstance(el, dict) and (_bool(el.get("is_essential")) or _bool(el.get("essential"))):
        return True
    return False
...
def _collect_img_candidates(ctx: PageContext) -> List[Any]:
    ...
    Candidatos primarios: <img>. Secundarios (opcional si el extractor los provee):
      - bg_images: CSS background-image con flags ({url, is_image_of_text, is_logo, ...})
      - canvas_nodes: <canvas> con drawText detectado (canvas_has_text)
      - svgs: inline <svg> (si trae <text> no es imagen de texto; si no, requeriría visión)
    """
...
def _compute_counts_raw(ctx: PageContext) -> Dict[str, Any]:
    """
    1.4.5 (AA): Si la tecnología permite la presentación visual, usar TEXTO en lugar de IMÁGENES DE TEXTO.
    Excepciones: logotipos, casos esenciales y contenido de texto cuya presentación puede ser personalizada por el usuario.
    RAW:
      - usa flags del extractor: is_image_of_text / ocr_text_present / detected_text_area_ratio
      - heurísticos suaves (filename/clases/alt) para marcar RIESGOS
      - soporta bg_images/canvas_nodes si se exponen en el contexto
    """
    candidates = _collect_img_candidates(ctx)
    ...
    for el in candidates:
        ...
        if not is_iot:
            continue  # no hay indicio suficiente

        flagged_image_of_text += 1

        # Excepciones
        if _is_customizable_exception(el):
            exempt_customizable += 1
            continue
        if _is_essential_exception(el):
            exempt_essential += 1
            continue
        ...
        offenders.append({
            ...
            "src": _get_attr_str(el, "src")[:180],
            "alt": _get_attr_str(el, "alt")[:120],
            "reason": "Imagen de TEXTO detectada sin excepción aplicable (usar HTML/CSS en su lugar)."
        })

    # --- background-image (opcional)
    for bg in bg_images:
        url = str(bg.get("url") or bg.get("image") or "")
        is_logo = _bool(bg.get("is_logo"))
        if is_logo:
            exempt_logos += 1
            continue
        ...
        if not is_iot:
            continue
        if _bool(bg.get("is_customizable")):
            exempt_customizable += 1
            continue
        if _bool(bg.get("is_essential")):
            exempt_essential += 1
            continue
        violations += 1
        offenders.append({
            "type": "bg_image",
            "url": url[:180],
            "reason": "Imagen de TEXTO en background sin excepción aplicable."
        })
    ...
    for cn in canvas_nodes:
        ...
        if _bool(cn.get("is_logo")):
            exempt_logos += 1
            continue
        if _bool(cn.get("is_customizable")):
            exempt_customizable += 1
            continue
        if _bool(cn.get("is_essential")):
            exempt_essential += 1
            continue
        violations += 1
        offenders.append({
            "type": "canvas",
            "id": cn.get("id",""),
            "reason": "Texto renderizado en <canvas> sin alternativa basada en HTML/CSS."
        })
    ...
    for s in svgs:
        ...
        if has_text_nodes:
            svg_text_ok += 1
        # Si no tiene <text>, no lo contamos como violación automática (difícil sin visión)
    ...
    details: Dict[str, Any] = {
        ...
        "exempt_customizable": exempt_customizable,
        "exempt_essential": exempt_essential,
        "risky_heuristics": risky_heuristics,
        "svg_text_ok": svg_text_ok,
        "ok_ratio": ok_ratio,
        "offenders": offenders,
        "note": (
            "RAW: 1.4.5 requiere usar TEXTO en lugar de IMÁGENES DE TEXTO, salvo casos de logotipos, "
            "presentación personalizable por el usuario o cuando la presentación específica es esencial. "
            "Se usan flags de extractor (is_image_of_text/ocr/area) y heurísticas de archivo/clases para marcar riesgos."
        )
    }
    return details
...
def _compute_counts_rendered(rctx: Optional[PageContext]) -> Dict[str, Any]:
    ...
    """
    if rctx is None:
        return {"na": True, "note": "No se proveyó rendered_ctx para 1.4.5; no se pudo evaluar en modo renderizado."}
    ...
    # Re-pondera con OCR
    # Si OCR detecta texto (conf>0.6 y área>0.3) en imágenes no exentas → refuerza violación
    extra_viol = 0
    for it in ocr_items:
        ...
        if len(text) >= 2 and conf >= 0.6 and area >= 0.3:
            # intenta ver si es logo por url/alt heurístico
            url = str(it.get("src") or it.get("url") or "")
            fake_el = {"src": url, "alt": it.get("alt",""), "class": it.get("class", [])}
            if _is_logo_like(fake_el) or _is_customizable_exception(fake_el) or _is_essential_exception(fake_el):
                continue
            extra_viol += 1
            d.setdefault("offenders", []).append({
                ...
                "confidence": conf,
                "area_ratio": area,
                "reason": "OCR en render detecta texto prominente en imagen."
            })
...
def _ai_review(details: Dict[str, Any], html_sample: Optional[str] = None) -> Dict[str, Any]:
    """
    IA: propone reemplazar imágenes de texto por HTML/CSS:
      - Sustituir banners/CTAs por contenedores con texto real (webfonts, letter-spacing, text-shadow).
      - Mantener logotipos como excepción.
      - Si es esencial, documentar y ofrecer alternativa accesible (texto equivalente y escalable).
    """
    if ask_json is None:
    ...
    ctx_json = {
        "summary": {
            ...
            "exempt_customizable": details.get("exempt_customizable", 0),
            "exempt_essential": details.get("exempt_essential", 0),
    ...
    prompt = (
        "Actúa como auditor WCAG 1.4.5 (Imágenes de texto). "
        "Para cada offender, sugiere cómo reemplazar por texto real: "
        "1) HTML semántico + CSS (webfonts, text-shadow, gradients, outline) para mantener estilo; "
        "2) Evitar rasterizar texto en PNG/JPG; "
        "3) Si el caso es esencial, documenta justificación y provee alternativa en texto; "
        "4) Mantener logotipos como excepción permitida. "
...
def run_1_4_5(
    ...
    return CriterionOutcome(
        ...
        title=meta.get("title", "Imágenes de texto"),
        source=src,
